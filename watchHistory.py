...
class WatchHistory:
        ...
        for i, mid in enumerate(movie_ids):
            ...
            if mid <= 0:
                raise InvalidWatchHistoryError(
                    f"ID фильма на позиции {i + 1} должен быть положительным. Получено: {mid}")
    ...
    def __init__(self, user_id: int, movie_ids: list[int]):
        self.user_id = self.check_watchhistory_user_id(user_id)
        self.movie_ids = self.check_watchhistory_movie_ids(movie_ids)
