...
def check_viewing_watch_time(watch_time):
    ...
    if not isinstance(watch_time, str):
        raise InvalidViewingError("Время просмотра должно быть строкой (например, '01:23:45' или '2025-04-05T20:30')")
    if watch_time.strip() == "":
...
class Viewing:
    def __init__(self, user_id: int, movie_id: int, watch_time: str):
        self.user_id = check_viewing_user_id(user_id)
        self.movie_id = check_viewing_movie_id(movie_id)
        self.watch_time = check_viewing_watch_time(watch_time)
